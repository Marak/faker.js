...
function Random (faker, seed) {
   ...
   * @param {string} type The part of speech of the word. Either 'adjective', 'noun', 'verb'. If null, will return any word
   */
  this.word = function randomWord (type) {
    ...
    if (type === "adjective") {
      wordMethods = wordMethods.filter(method => {
        var lcMethod = method.toLowerCase();
        return lcMethod.includes("adjective") || lcMethod.includes('color');
      });
    } else if (type === "noun") {
      wordMethods = wordMethods.filter(method => method.toLowerCase().includes('noun'));
    } else if (type === "verb") {
      wordMethods = wordMethods.filter(method => method.toLowerCase().includes('verb'));
    }
   ...
   * @param {string} type The part of speech of the word. Either 'adjective', 'noun', 'verb'. If null, will return any word
   */
  this.words = function randomWords (count, type) {
    var words = [];
    if (typeof count === "undefined") {
      count = faker.random.number({min:1, max: 3});
    }
    for (var i = 0; i<count; i++) {
      words.push(faker.random.word(type));
    }
    return words.join(' ');
  }
