...
function Random (faker, seed) {
  ...
  /**
   * returns a random array of whatever you pass it including faker functions
   *
   * @method faker.random.arrayOf
   * @param {object} what what to fill each array with
   * @param {number} limit maximum length of array to create
   */
  this.arrayOf = function (what, limit) {
    let array = []
    limit = faker.random.number({min: 1, max: limit | 3})
    for (let i = 0; i < limit; i++) {
      let fake = Object.assign({}, what)
      Object.entries(fake).forEach((kv, i, a) => {
        if (typeof kv[1] == 'function') fake[kv[0]] = kv[1]()
        if (i === a.length-1) array.push(fake)
      })
    }
    return array
  }
