...
function Random (faker, seed) {
  ...
  /**
   * Runs the given callback random times
   *
   * @method faker.random.times
   * @param {(number|object|function)} options
   * @param {function} callback
   */
  this.times = function randomTimes(options, callback) {
    if (typeof callback === "undefined") {
      // Assign the function so all.functional.js tests pass
      callback = options || function (i) { return i; };
      options = { min: 1, max: 10 };
    }

    var count = faker.random.number(options);
    var result = [];
