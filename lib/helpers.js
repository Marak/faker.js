...
// generates a UK postal code of a given format
exports.ukPostalCode = function (format) {
    // default format is LLD DDL
    if (format === undefined) {
        format = 'LLD DLL';
    }
    ...
    for (var i = 0; i < format.length; i++) {
        if (format[i] == 'L') {
            ukPostalCode += letters[Math.floor(Math.random() * letters.length)];
        }
        else if (format[i] == 'D') {
            ukPostalCode += Math.floor(Math.random() * 10);
        }
        else {
            ukPostalCode += ' ';
        }
