...
var Internet = function (faker) {
  ...
  self.email = function (firstName, lastName, provider, useFreeProviders) {
      var defaultProvider = faker.internet.domainName();
      var resolvedProvider = defaultProvider;
      if (useFreeProviders) {
        defaultProvider = faker.random.arrayElement(faker.definitions.internet.free_email);
      }
      if (provider) {
        if (typeof provider === 'string' && provider.length > 0) {
          resolvedProvider = provider;
        } else if (Array.isArray(provider) && provider.length > 0) {
          resolvedProvider = faker.random.arrayElement(provider);
        }
      }
      return faker.helpers.slugify(faker.internet.userName(firstName, lastName)) + "@" + resolvedProvider;
  };
